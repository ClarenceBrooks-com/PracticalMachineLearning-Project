...
This summarizes an attempt to build a simple machine learning algorithm, 
with the goal of predicting from a sample data set how well a barbell lift 
was performed.
...
NOTE:  The training and testing csv files are assumed to have been 
       already downloaded and in the current working directory.
```{r}
...
## Convert all of the data columns to numeric.
for(i in 8:159){
...
This generated a number of columns with NAs.  
In fact, any columns with NAs were almost entirely full of NA entries;
```{r}
...
Next, the plots were visually inspected.  Any variable where the boxes for a single variable had some significant differences were then further inspected via a stacked histogram, such as this one for the accel_belt_z variable:

```{r, warning = FALSE}
ggplot(data = training, aes(x = accel_belt_z, fill = classe)) + geom_histogram()
```

Those that were determined to be potentially useful were then noted, 
and ultimately used in the final model.

## Cross validation

In order to properly cross-validate the model, the following procedure was repeated 10 times:

First, the initial training data was randomly split into two subsets - one for training, one for testing, like so:
```{r}
...
Once the 10 iterations were complete, the accuracies were averaged together to get a single estimate of the accuracy, from which an estimate of the out-of-sample error could be determined.
